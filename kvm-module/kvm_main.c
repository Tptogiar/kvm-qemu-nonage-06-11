...
static int kvm_handle_exit(struct kvm_run *kvm_run, struct kvm_vcpu *vcpu)
{
	...
	if (exit_reason < kvm_vmx_max_exit_handlers
		...
		return kvm_vmx_exit_handlers[exit_reason](vcpu, kvm_run);   // 跳转到handlers数组
	else {
...
static long kvm_dev_ioctl(struct file *filp,   // 接口处理函数
...
static __init int kvm_init(void)
{
	...
	r = misc_register(&kvm_dev);   // 注册到/dev/kvm中
	if (r) {
